    ...
    with open(os.path.join(args.dump_root, 'test_flow.txt'), 'w') as tf:
        for i in frame_list:
            ...
            for o in range(-half_offset, half_offset+1):
                ...
                curr_seg_path = '/mnt/lustre/dingmingyu/Research/geo/pspnet/exp/cityscapes/psp50_kitti/result_flow/epoch_200/val/ss/gray/%.6d_%.2d.png' % (i, 10+o)
                curr_img = cv2.imread(curr_img_path)
                curr_seg = cv2.imread(curr_seg_path)
                if o==0:
                ...
                curr_seg = cv2.resize(curr_seg, (args.img_width, args.img_height))
                print(curr_img.shape, curr_seg.shape)
                #curr_seg = np.expand_dims(curr_seg,axis=2)
                curr_img = np.concatenate((curr_img, curr_seg[:,:,0:1]), axis=2)
            ...
            dump_img_file = dump_dir + '/%.6d.npy' % i
            np.save(dump_img_file, img_seq.astype(np.uint8))
